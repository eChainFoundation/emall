...
<form id="main_form" name="main_form" action="<%=basePath %>Jrj/updateUser" method="post" onsubmit="if(checkForm($(this))) doSubmit($(this)); return false;">
	<div class="top-guide">
...
<section style="padding-top:0">
	<table class="table table-bordered">
		<tbody>
		<input type="hidden" name="id" required="true" value="${user.id}" id="id" />
			<tr>
				<td style="width:200px">员工名</td>
      			<td>
      				<input type="text" maxlength="50" name="realName" required="true" value="${user.realName}" id="userName" />
      			</td>
			</tr>
			<tr>
      			...
      			<td>
      				<div class="cover-area" style="width: 500px">
						...
						<p class="img-area cover-bd">
							<img class="img" src="<%=basePath%>${user.userPhoto}"/>
							<input id="logo" class="cover-input" value="${user.userPhoto}" name="userPhoto" type="hidden" width="100px"/>
						</p>
					</div>	
      			</td>
			</tr>
			<tr>
				<td style="width:200px">二维码</td>
      			<td>
      				<img class="img" src="<%=basePath%>${user.qrcode}" width="100px"/>
      				<input type="hidden" name="qrcode" value="${user.qrcode}" id="qrcode">
      			</td>
			</tr>
			<tr>
				<td style="width:200px">出生年月</td>
      			<td>
      				<input type="text" name="birthday" value="${user.birthday}" id="birthday" required="true">
      			</td>
			</tr>
			<tr>
				<td style="width:200px">入场时间</td>
      			<td>
      				<input type="text" name="workDate" value="${user.workDate}" id="workDate" required="true">
      			</td>
			</tr>
			<tr>
				<td style="width:200px">是否是项目管理人员</td>
      			<td><select name="status" id="status">
						<option value="1" <c:if test="${user.status eq '1'}">selected</c:if>>是</option>
						<option value="0" <c:if test="${user.status eq '0'}">selected</c:if>>否</option>
					</select>
				<font color="red">*</font> 
				</td>
			</tr>
			<tr>
				<td style="width:200px">职务</td>
      			<td>
      				<input type="text" name="zhiwu" value="${user.zhiwu}" id="zhiwu">
      			</td>
			</tr>
			...
			<tr>
				<td style="width:200px">是否带班教育</td>
      			<td><select name="isDaiban" id="isDaiban">
						<option value="1" <c:if test="${user.isDaiban eq '1'}">selected</c:if>>是</option>
						<option value="0" <c:if test="${user.isDaiban eq '0'}">selected</c:if>>否</option>
					</select>
				</td>
			</tr>
			<tr>
				<td style="width:200px">施工班组</td>
      			<td><select name="departmentId">
      				<option value="${user.departmentId}">${user.department}</option>
			...
			<tr>
				<td style="width:200px">施工班组长</td>
      			<td>
      				<input type="text" name="leader" value="${user.leader}" id="leader">
      			</td>
			</tr>
			<tr>
				<td style="width:200px">宿舍号</td>
      			<td>
      				<input type="text" name="dormitoryNo" value="${user.dormitoryNo}" id="dormitoryNo" >
      			</td>
			</tr>
			...
			<tr>
				...
				<td><select name="isSafeStudy" id="isSafeStudy">
						<option value="1" <c:if test="${user.isSafeStudy eq '1'}">selected</c:if>>是</option>
						<option value="0" <c:if test="${user.isSafeStudy eq '0'}">selected</c:if>>否</option>
					</select>
				<font color="red">*</font> 
				</td>
			</tr>
			<tr>
				...
				<td><select name="morningSaveStudy" id="morningSaveStudy">
						<option value="1" <c:if test="${user.morningSaveStudy eq '1'}">selected</c:if>>是</option>
						<option value="0" <c:if test="${user.morningSaveStudy eq '0'}">selected</c:if>>否</option>
					</select>
				<font color="red">*</font> 
				</td>
			</tr>
			<tr>
				...
				<td><select name="citizenState" id="citizenState">
						<option value="1" <c:if test="${user.citizenState eq '1'}">selected</c:if>>有</option>
						<option value="0" <c:if test="${user.citizenState eq '0'}">selected</c:if>>无</option>
					</select>
				<font color="red">*</font> 
				</td>
			</tr>
			<tr>
				<td>市民卡号：</td>
				<td>
					<input type="text" name="citizenCard" value="${user.citizenCard}" id="citizenCard" > 
				</td>
			</tr>
			<tr>
				<td>安全帽编号：</td>
				<td>
					<input type="text" name="anquanmaoNo" value="${user.anquanmaoNo}" id="anquanmaoNo" > 
				</td>
			</tr>
			<tr>
				<td>违规：</td>
				<td>
					<input type="text" name="weigui" value="${user.weigui}" id="weigui" > 
				</td>
			</tr>
			<tr>
				<td></td>
