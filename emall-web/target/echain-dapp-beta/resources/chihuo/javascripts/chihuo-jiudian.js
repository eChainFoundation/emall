...
(function pullPagePullImplementation($) {
  ...
  function gotPullDownData(event, data) {
	...
	$.get("/Chihuo/jiudianParam?currentPage="+(++currentPage)+"&param="+param,function(data){
		if(data == "") {
  ...
  function gotPullUpData(event, data) {
    var i,
        ...
        newContent = "";
    var param = $('.screenAdd a.active').text();
	$.get("/Chihuo/jiudianParam?currentPage="+(++currentPage)+"&param="+param,function(res){
		if(res == "") {
		 	if(endPage == 0) {
				newContent = "<li><span class='listLast'>加载完毕！</span><li>";
				endPage = 1;
...
(function shortPullPagePullImplementation($) { 
  ...
  alert(111);
    ...
    $.ajax({
		type: "GET",
		url: "/Chihuo/jiudianParam?currentPage="+(++currentPage)+"&param="+param,
		data: { },
		dataType: "string",
