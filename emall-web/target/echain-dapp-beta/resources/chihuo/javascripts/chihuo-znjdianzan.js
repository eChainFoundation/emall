...
(function pullPagePullImplementation($) {
  ...
  function gotPullDownData(event, data) {
	...
	$.get("/MyZounongjia/dianzanMore?currentPage="+(++currentPage),function(data){
		if(data == "") {
			newContent = "加载完毕！"
		} else
			newContent = data;
  ...
  function gotPullUpData(event, data) {
	...
	$.get("/MyZounongjia/dianzanMore?currentPage="+(++currentPage),function(res){
		if(res == "") {
		 	if(endPage == 0) {
...
(function shortPullPagePullImplementation($) { 
  ...
  function gotPullDownData(event, data) {
    ...
    var param = $('.screenAdd a').text();
    alert(param);
	$.get("/Chihuo/youhuiParam?currentPage="+(++currentPage)+"&param="+param,function(data){
		if(data == "") {
			newContent = "加载完毕！"
		} else
			newContent = data;
  ...
  function gotPullUpData(event, data) {
    ...
    $.ajax({
		type: "GET",
		url: "/Chihuo/dianpinParam?currentPage="+(++currentPage)+"&param="+param+"&flag="+flag,
		data: { },
		dataType: "string",
