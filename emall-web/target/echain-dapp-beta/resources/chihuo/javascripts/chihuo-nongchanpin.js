...
(function pullPagePullImplementation($) {
  ...
  function gotPullDownData(event, data) {
	...
	$.get("/Zounongjia/nongchanpinParam?currentPage="+(++currentPage)+"&shopTitle="+shopTitle+"&productType="+productType+"&param="+param,function(data){
		if(data == "") {
			newContent = "加载完毕！"
		} else
			newContent = data;
  ...
  function gotPullUpData(event, data) {
	...
	$.get("/Zounongjia/nongchanpinParam?currentPage="+(++currentPage)+"&shopTitle="+shopTitle+"&productType="+productType+"&param="+param,function(res){
		if(res == "") {
		 	if(endPage == 0) {
