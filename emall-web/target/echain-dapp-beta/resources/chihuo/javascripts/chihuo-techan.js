...
(function pullPagePullImplementation($) {
  ...
  function gotPullDownData(event, data) {
	...
	$.get("/Chihuo/techanParam?currentPage="+(++currentPage)+"&param="+param,function(data){
		if(data == "") {
			newContent = "加载完毕！"
		} else
			newContent = data;
  ...
  function gotPullUpData(event, data) {
    var i,
        ...
        newContent = "";
    var param = $(".newsTabs li.active").text();
	$.get("/Chihuo/techanMore?currentPage="+(++currentPage)+"&cat="+param,function(res){
		if(res == "") {
		 	if(endPage == 0) {
...
(function shortPullPagePullImplementation($) { 
  ...
  function gotPullDownData(event, data) {
    ...
    var param = $('.screenAdd a').text();
    alert(param);
	$.get("/Chihuo/huiyuankaParam?currentPage="+(++currentPage)+"&param="+param,function(data){
		if(data == "") {
			newContent = "加载完毕！"
		} else
			newContent = data;
  ...
  alert(111);
    ...
    $.ajax({
		type: "GET",
		url: "/Chihuo/huiyuankaParam?currentPage="+(++currentPage)+"&param="+param,
		data: { },
		dataType: "string",
