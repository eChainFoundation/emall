...
(function pullPagePullImplementation($) {
  ...
  function gotPullDownData(event, data) {
	...
	$.get("/Zounongjia/huodongParam?currentPage="+(++currentPage)+"&shopTitle="+shopTitle+"&param="+param,function(data){
		if(data == "") {
			newContent = "加载完毕！"
		} else
			newContent = data;
  ...
  function gotPullUpData(event, data) {
	...
	$.get("/Zounongjia/huodongParam?currentPage="+(++currentPage)+"&shopTitle="+shopTitle+"&param="+param,function(res){
		if(res == "") {
		 	if(endPage == 0) {
