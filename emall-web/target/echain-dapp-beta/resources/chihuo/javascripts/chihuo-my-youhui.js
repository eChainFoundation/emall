...
(function pullPagePullImplementation($) {
  ...
  function gotPullDownData(event, data) {
	...
	$.get("/MyChihuo/youhuiMore?currentPage="+(++currentPage),function(data){
		if(data == "") {
			newContent = "加载完毕！"
		} else
			newContent = data;
  ...
  function gotPullUpData(event, data) {
	...
	$.get("/MyChihuo/youhuiMore?currentPage="+(++currentPage),function(res){
		if(res == "") {
		 	if(endPage == 0) {
...
(function shortPullPagePullImplementation($) { 
  ...
  alert(111);
    ...
    $.ajax({
		type: "GET",
		url: "/Chihuo/youhuiParam?currentPage="+(++currentPage)+"&param="+param,
		data: { },
		dataType: "string",
