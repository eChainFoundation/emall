...
(function pullPagePullImplementation($) {
  ...
  function gotPullDownData(event, data) {
	...
	$.get("/MyZounongjia/myhuodongMore?currentPage="+(++currentPage)+"&param="+param,function(data){
		if(data == "") {
			newContent = "加载完毕！"
		} else
			newContent = data;
  ...
  function gotPullUpData(event, data) {
	...
	$.get("/MyZounongjia/myhuodongMore?currentPage="+(++currentPage)+"&param="+param,function(res){
		if(res == "") {
		 	if(endPage == 0) {
