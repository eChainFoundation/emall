...
function addFile(image) {
    var el = $('<li class="post file" data-post-id="' + image.id + '">'
		...
		+ '<dl class="form data">'
			...
			//+ '<dt class="tags">标签</dt><dd class="tags"><input type="text" class="text" placeholder="多个标签之间用空格分开?" name="images[' + image.id + '][tags]" /></dd>'
		+ '</dl>'
		+ '<ul class="action">'
...
function initUpload(el,count, op,url,delurl,swfurl) {
    ...
    el.uploadify({
        ...
        'onUploadStart': function (file) {
            ...
            if ($("#bsubmit").length == 0) { $('#file_upload-button').parent().append('   <button id="bsubmit" type="submit" data-loading-text="提交�?..." class="btn">保存</button>') }
