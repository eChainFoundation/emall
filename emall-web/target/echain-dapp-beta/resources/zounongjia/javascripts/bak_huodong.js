...
(function pullPagePullImplementation($) {
  ...
  function gotPullDownData(event, data) {
	...
	$.get("/Activity/huodongParam?currentPage="+(++currentPage)+"&userId="+userId+"&shopTitle="+shopTitle+"&area="+area+"&cat="+cat+"&paixu="+paixu,function(data){
		if(data == "") {
			newContent = "<li class=\"pageList clearfix\">加载完毕！</li>"
		} else
			newContent = data;
    });
