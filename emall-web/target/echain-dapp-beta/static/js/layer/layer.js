	...
	CLASS.prototype = {
		...
		space : function(){ //层容器
			...
			frame = [
				...
				'<iframe id="xubox_iframe" name="xubox_iframe" class="xubox_iframe" frameborder="0" onload="javascript:$(\'.xubox_layer\').find(\'.xubox_iframe\').removeClass(\'xubox_load\');" src="' + this.iframe.src + '"></iframe>',				
			...
			shadeStyle = "z-index:"+ zIndex +"; background:"+ config.shade[1] +"; opacity:"+ config.shade[0] +";";
			config.shade[2] && (shade = '<div id="xubox_shade' + times + '" class="xubox_shade xubox_shade_' + config.skin + '" style="'+ shadeStyle +'"></div>');	
			var borderStyle = "z-index:"+ (zIndex-1) +";  top:-"+ config.border[0] +"px; left:-"+  config.border[0] +"px;";
		...
		pagetype : function(){ //调整layer			
			...
			layerE.animate({opacity : 1 },fadeTime);
			this.move();
		},
